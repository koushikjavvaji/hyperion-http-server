...
void start_kqueue_server(int port)
{
    ...
    std::vector<Connection *> connections(MAX_FD, nullptr);

    struct kevent events[MAX_EVENTS];
    std::cout << "Worker running on port " << port << std::endl;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (fd == server_fd)
            {
                while (true)
                {
                    ...
                    hyperion::Metrics::instance().record_connection();

                    struct kevent ce;
                    EV_SET(&ce, client_fd, EVFILT_READ,
                           EV_ADD | EV_CLEAR, 0, 0, NULL);
            ...
            if (events[i].flags & EV_EOF)
            {
                ...
                connections[fd] = nullptr;
                hyperion::Metrics::instance().record_disconnection();
                if (pool_idx > 0)
                    pool[--pool_idx] = conn;
                else
                    delete conn;
